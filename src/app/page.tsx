...
import {
  ...
  ArrowRight, ChevronDown, X, Terminal, Zap, Globe, Crosshair, Gamepad2
} from "lucide-react";
...
// Lazy-load the Range / game view — only pulled in when the visitor enters it
...
export default function Portfolio() {
  ...
  // View switch — "dossier" = normal portfolio · "range" = the shooting-gallery game
  const [view, setView] = useState<"dossier" | "range">("dossier");
...
Contact: abhinavkuwork@gmail.com | linkedin.com/in/abhinavku6129`;
  ...
  return (
    <div className="relative">
      ...
      {/* Custom Cursor — page shot FX disabled while the Range owns shooting */}
      <CustomCursor onShot={handleShot} bullets={bullets} sparks={sparks} effectsEnabled={view === "dossier"} />

      {/* ── RANGE MODE — full-screen game overlay ─────────────────── */}
      <AnimatePresence>
        {view === "range" && (
          <Suspense
            fallback={
              <div className="fixed inset-0 z-[500] flex items-center justify-center" style={{ background: "#080C12" }}>
                <span className="font-mono-custom text-xs tracking-[0.4em] uppercase text-white/40 animate-pulse">
                  Loading Range…
                </span>
              </div>
            }
          >
            <GameView onExit={() => setView("dossier")} />
          </Suspense>
        )}
      </AnimatePresence>
      ...
      <nav className="fixed inset-x-0 top-0 z-50 border-b border-white/5" style={{ background: "rgba(8,12,18,0.92)", backdropFilter: "blur(12px)" }}>
        <div className="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {/* Enter the shooting-gallery game view */}
            <button
              onClick={() => setView("range")}
              title="Enter Range Mode — unlock this portfolio by clearing shooting stages"
              className="group hidden sm:flex items-center gap-2 font-mono-custom text-xs tracking-widest uppercase px-4 py-2 border transition"
              style={{ borderColor: "#F5A62366", color: "#F5A623", background: "#F5A6230A" }}
            >
              <Crosshair className="w-4 h-4 group-hover:rotate-90 transition-transform duration-500" />
              Range Mode
            </button>
            {/* Mobile entry to Range Mode */}
            <button
              onClick={() => setView("range")}
              title="Enter Range Mode"
              className="sm:hidden p-1 transition"
              style={{ color: "#F5A623" }}
            >
...
function CustomCursor({ onShot, bullets, sparks, effectsEnabled = true }: {
  ...
  // Mirror into a ref so the mount-once listeners see the live value
  ...
  useEffect(() => {
    ...
    const down = (e: MouseEvent) => {
      setClicking(true);
      // While the Range game is active it owns all shot audio/FX
